import { Controller, Get, Logger, Optional, Inject } from '@nestjs/common';
import { ApiTags, ApiOperation } from '@nestjs/swagger';
import { CacheService } from '../cache/cache.service';
import { SocketGateway } from '../websocket/socket.gateway';
import { AiService } from '../modules/ai/ai.service';
import { LlmProviderService } from '../modules/multi-agent/llm/llm-provider.service';
...
export class MetricsController {
  ...
  constructor(
    ...
    @Optional() private readonly llmProviderService?: LlmProviderService,
  ) {}
  ...
  async getMetrics(): Promise<string> {
    ...
    // LLM Provider V2 metrics
    if (this.llmProviderService) {
      const usage = this.llmProviderService.getTokenUsage();
      lines.push('# HELP llm_tokens_prompt_total Total prompt tokens consumed by LLM providers');
      lines.push('# TYPE llm_tokens_prompt_total counter');
      lines.push(`llm_tokens_prompt_total ${usage.prompt}`);

      lines.push('# HELP llm_tokens_completion_total Total completion tokens generated by LLM providers');
      lines.push('# TYPE llm_tokens_completion_total counter');
      lines.push(`llm_tokens_completion_total ${usage.completion}`);

      lines.push('# HELP llm_tokens_total Cumulative total tokens used across all agents');
      lines.push('# TYPE llm_tokens_total counter');
      lines.push(`llm_tokens_total ${usage.total}`);

      lines.push('# HELP llm_requests_total Total HTTP requests sent to LLM providers');
      lines.push('# TYPE llm_requests_total counter');
      lines.push(`llm_requests_total ${usage.requestCount}`);
    }
